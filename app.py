...
import os
# ─── Groq API ─────────────────────────────────────────────
# Récupère la clé depuis les variables d'environnement pour la sécurité
api_key = os.environ.get("GROQ_API_KEY", "VOTRE_CLE_ICI")
...
try:
    ...
    print(f"[OK] CSV chargé : {len(df)} lignes")
    print(f"     Colonnes   : {list(df.columns)}")
except Exception as e:
    df = None
    print(f"[AVERT.] water.csv introuvable : {e}")
...
def get_sensor():
    if df is not None:
        ...
        return {
            ...
            "ec":              round(float(row.get("Conductivity", 450)) / 200, 2),
            "nitrogen":        round(float(row.get("Organic_carbon", 14)) * 10, 1),
            "temperature":     round(random.uniform(18, 26), 1),
            "phosphorus":      round(float(row.get("Sulfate", 350)) / 7, 1),
            "potassium":       round(float(row.get("Hardness", 200)) * 1.2, 1),
...
def get_alerts(data):
    ...
    if data["ph"] < 5.5 or data["ph"] > 7.5:
        alerts.append(f"pH anormal : {data['ph']} — Ajuster maintenant")
    if data["turbidity"] > 4.0:
        alerts.append(f"Turbidité élevée : {data['turbidity']} NTU")
    if data["chloramines"] > 8.0:
        alerts.append(f"Chloramines élevées : {data['chloramines']} mg/L")
    if data["battery"] < 20:
...
def api_chat():
    try:
        ...
        prompt = f"""
Tu es AmanBot, expert qualité d'eau pour agriculture hydroponique verticale.
Réponds en français. Court et pratique. Maximum 3 points.
